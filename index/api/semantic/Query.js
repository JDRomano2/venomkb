...
(function(ontology) {
  ...
  ontology["VenomSeqData"] = "VenomSeqData";
})(ontology || (ontology = {}));
var properties;
(function(properties) {
...
class NeoAdapter {
  constructor(user, password, uri) {
    ...
    this.uri = "";
    this.driver = neo4j_module.v1.driver(
      uri,
      neo4j_module.v1.auth.basic(this.user, this.password)
    );
  }
}
...
class Query {
  ...
  async findPropertyKeys() {
    ...
    try {
      const resultPromise = await this.session.writeTransaction(tx =>
        tx.run(query)
      );
  ...
  async collectSelect() {
    if (typeof this.json.select == "string") {
      ...
      if (aggregate) {
        if (
          this.json.aggregate.count &&
          this.json.aggregate.count.class == object
        ) {
          obj[object] = null;
          this.select.push(obj);
        } else if (
          this.json.aggregate.distinct &&
          this.json.aggregate.distinct.class == object
        ) {
          obj[object] = null;
          this.select.push(obj);
        } else {
      ...
      this.json.select.forEach(element => {
        if (typeof element == "string") {
          if (this.json.aggregate) {
            ...
            if (
              this.json.aggregate.count &&
              this.json.aggregate.count.class == element
            ) {
              found = true;
            }
            if (
              this.json.aggregate.distinct &&
              this.json.aggregate.distinct.class == element
            ) {
              found = true;
            }
            if (found) {
   ...
   * @memberof Query
   */
  async findShortestPathBetween2(class1, class2) {
    // const session = this.neo4j_module.v1_adapter.session
    // const driver = this.neo4j_module.v1_adapter.driver
    // case of direct relation ship
    const query_relation =
      "MATCH(c1: OntologyClass { name: '" +
      class1 +
      "'}), (c2: OntologyClass { name: '" +
      class2 +
      "'}), p = shortestPath((c1) - [*] -> (c2)) RETURN p";
  ...
  buildQueryMatch() {
    ...
    if (this.ontologyClasses.length == 1) {
      query_match +=
        "(" +
        item[this.ontologyClasses[0]] +
        ":" +
        this.ontologyClasses[0] +
        ")";
    } else {
  ...
  buildReturn() {
    ...
    for (let i in this.select) {
      ...
      const ontology = Object.keys(select)[0];
      ...
      if (value == null && aggregate) {
        if (aggregate.count) {
          ...
          if (
            aggregate.distinct &&
            aggregate.count.class == aggregate.distinct.class
          ) {
